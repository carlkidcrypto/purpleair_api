#!/usr/bin/env python3
"""Audit purpleair_api documentation for common issues and typos.

This script scans documentation files (*.rst, *.md) and Python docstrings under
``purpleair_api/`` for known issues:
...
CHECKS: list[tuple[str, re.Pattern[str], str | None]] = [
    ...
    (
        "Incorrect class casing 'PurpleairAPI' (should be 'PurpleAirAPI')",
        re.compile(r"\bPurpleairAPI\b"),
        "PurpleAirAPI",
    ),
    (
        "Incorrect class casing 'PurpleairReadAPI' (should be 'PurpleAirReadAPI')",
        re.compile(r"\bPurpleairReadAPI\b"),
        "PurpleAirReadAPI",
    ),
    (
        "Incorrect class casing 'PurpleairWriteAPI' (should be 'PurpleAirWriteAPI')",
        re.compile(r"\bPurpleairWriteAPI\b"),
        "PurpleAirWriteAPI",
    ),
    (
        "Incorrect class casing 'PurpleairLocalAPI' (should be 'PurpleAirLocalAPI')",
        re.compile(r"\bPurpleairLocalAPI\b"),
        "PurpleAirLocalAPI",
    ),
    (
        "Incorrect class casing 'PurpleairMatterConverter' (should be 'PurpleAirMatterConverter')",
        re.compile(r"\bPurpleairMatterConverter\b"),
...
def main() -> int:
    """CLI entrypoint."""
    parser = argparse.ArgumentParser(description="Audit purpleair_api documentation.")
    parser.add_argument(
        "--fix",
        action="store_true",
