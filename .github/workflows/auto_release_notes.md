...
1. Identify the release context:
   ...
   - Construct the PyPI URL: `https://pypi.org/project/purpleair_api/<pypi_version>/`
...
3. Group commits into themes. Use these categories (add others if clearly needed):
   ...
   - **API / Core Library** — behavior changes in the main library implementation (`purpleair_api/` Python sources, HTTP request handling, response parsing, authentication, sensor data logic).
   - **Tests** — additions or updates to test suites (`tests/`).
   - **Packaging** — publish flows, wheel/build/publish behavior, `setup.py`, `setup.cfg`.
   - **CI / Workflows** — changes under `.github/workflows/` or build pipeline configuration.
   - **Documentation** — changes to docs, `README.rst`, `HOWTOAI.md`, `CHANGELOG.md`, Sphinx files.
   - **Dependencies** — version bumps, lockfile updates, `requirements.txt` changes.
   - **Chores / Misc** — refactoring, code style, tooling, or anything that does not fit above.

   Infer category from BOTH commit metadata and changed paths. Do not rely only on commit titles. Prefer path-based classification when title-based and path-based signals disagree. If a commit touches multiple areas, place it in the most user-impacting section and mention secondary impact in the bullet text.
...
4. Build the release notes body:
   ...
   - Keep CI/dependency churn concise; prioritize concrete API, reliability, compatibility, and test-coverage outcomes.
   - If PR numbers are available, link them in the format `(#NNN)`.
   - If no commits fall in a category, omit that section entirely.
   - End with a **Full Changelog** line: `**Full Changelog**: https://github.com/carlkidcrypto/purpleair_api/compare/<base_tag>...<current_tag>`.
...
5. Update the GitHub release:
   ...
   - Endpoint: `PATCH /repos/carlkidcrypto/purpleair_api/releases/<release_id>`
...
**Full Changelog**: https://github.com/carlkidcrypto/purpleair_api/compare/<base_tag>...<current_tag>
```
